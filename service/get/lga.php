...
$conn =& $database;
$count = 0;
$data = new MultiForm($db_name, 'lgas','code', $conn);
$data->current_page = $page = (int)$params['page'] > 0 ? (int)$params['page'] : 1;
$query =
"SELECT lga.`name`, lga.`code`,
        (
          SELECT COUNT(*)
...
$cond = " WHERE lga.state_code = '{$params['state_code']}' ";
if (!empty($params['code'])) {
  $cond .= " AND lga.`code` = '{$conn->escapeValue($params['code'])}' ";
} else {
  if( !empty($params['search']) ){
    $params['search'] = $db->escapeValue(\strtolower($params['search']));
    $cond .= " AND (
      LOWER(lga.`code`) = '{$params['search']}'
      OR LOWER(lga.`name`) LIKE '%{$params['search']}%'
    ) ";
  }
}

$count = $data->findBySql("SELECT COUNT(*) AS cnt FROM :db:.:tbl: AS lga {$cond} ");
// echo $db->last_query;
...
$query .= $join;
$query .= $cond;
$sort = " ORDER BY LOWER(lga.`name`) = '-other', lga.`name` ASC ";
