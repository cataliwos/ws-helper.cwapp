...
$conn =& $database;
$count = 0;
$data = new MultiForm($db_name, 'states','code', $conn);
$data->current_page = $page = (int)$params['page'] > 0 ? (int)$params['page'] : 1;
$query =
"SELECT stt.`name`, stt.`code`,
        (
          SELECT COUNT(*)
          FROM :db:.`cities`
          WHERE state_code = stt.`code`
        ) AS cities,
        (
          SELECT COUNT(*)
...
$cond = " WHERE stt.country_code = '{$params['country_code']}' ";
if (!empty($params['code'])) {
  $cond .= " AND stt.`code` = '{$conn->escapeValue($params['code'])}' ";
} else {
  if( !empty($params['search']) ){
    $params['search'] = $db->escapeValue(\strtolower($params['search']));
    $cond .= " AND (
      LOWER(stt.`code`) = '{$params['search']}'
      OR LOWER(stt.`name`) LIKE '%{$params['search']}%'
    ) ";
  }
}

$count = $data->findBySql("SELECT COUNT(*) AS cnt FROM :db:.:tbl: AS stt {$cond} ");
// echo $db->last_query;
...
$query .= $join;
$query .= $cond;
$sort = " ORDER BY LOWER(stt.`name`) = '-other', stt.`name` ASC ";
